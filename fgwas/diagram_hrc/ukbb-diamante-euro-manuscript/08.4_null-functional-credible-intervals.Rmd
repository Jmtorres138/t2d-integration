...
missing.vec <- c("20_1","86_2","87_2","132_3","132_4","132_5",
                 "133_3","133_4","133_5","163_2","164_2")#,"243","244")
...
ggplot(data=block.df,aes(x=chunk.len,y=blk.len)) + 
  geom_point() + geom_abline(slope=1,intercept = 0,color="red") + theme_bw()  

ggplot(data=block.df,aes(x=chunk.len,y=blk.PPA)) + 
  geom_point() + geom_abline(slope=1,intercept = 0,color="red") + theme_bw()  
# Chunk length doesn't influence block PPA (regression p-value: 0.7002)

ggplot(data=block.df,aes(x=chunk.len,y=fcred.num)) + 
  geom_point() + geom_abline(slope=1,intercept = 0,color="red") + theme_bw()  
# Chunk length doesn't influence number of snps in functional credible set (regression p-value: p-value: 0.7427)

summary(block.df$chunk.len)
#   Min. 1st Qu.  Median    Mean 3rd Qu.    Max. 
# 546500 1000000 1000000  985600 1000000 1000000 
summary(filter(block.df,fcred.num!=0)$fcred.num)
#   Min. 1st Qu.  Median    Mean 3rd Qu.    Max. 
#   1.00    3.00   13.00   86.53   47.00 5143.00 
sub <- filter(block.df,chunk.len<1000000)
dim(sub)[1] # 28
summary(sub$chunk.len)
# Min. 1st Qu.  Median    Mean 3rd Qu.    Max. 
# 546500  710800  757500  790100  913800  983300 
summary(sub$fcred.num) # Includes 243 and 244 

sum(block.df$fcred.num==1)
# 56
sum(block.df$fcred.num==2)
# 30
sum(block.df$fcred.num==3)
# 21
sum(block.df$fcred.num==4)
# 15
sum(block.df$fcred.num<=10)
# 182
> 
...
#summary(block.df$fcred.num)
#   Min. 1st Qu.  Median    Mean 3rd Qu.    Max. 
#   1.00    3.00   13.00   88.39   48.00 5143.00 
#plot(block.df$blk.len,block.df$fcred.len)
#abline(a=0,b=1)

#59307725
#1 chr2 58807724 59807724
```
