...
write.table(loc.df,file=work.dir%&%"t2d-loci-regions.txt",sep="\t",quote=FALSE,
            row.names=F,col.names=F)

```


# Write loci-partitioned BED file 
...
write.table(bed.df,file=work.dir%&%"loci-partition.bed",sep="\t",quote=FALSE,
            row.names=F,col.names=F)
...
#t <- disjoin.gr[disjoin.gr %over% loci.gr]
#for (i in 1:length(loci.gr)){
  #print(loci.gr[i])
#  check <- disjoin.gr[disjoin.gr %over% loci.gr[i]]
#  if (length(check)>1){
#    print(i)
    #print("loci.gr")
    #print(loci.gr[i])
...
#}
check.df <- bed.df
check.df$Start <- check.df$Start+1
check.df$End <- check.df$End+1

t <- filter(check.df,Start%in%loc.df$Start,End%in%End)
str(t)

#plotGRanges(loci.gr,range=GRanges("chr8",IRanges(113565, 146300622)))

#    CHR     Start       End chrom    range
#1  chr7  31228453 116995666     7 85767213
