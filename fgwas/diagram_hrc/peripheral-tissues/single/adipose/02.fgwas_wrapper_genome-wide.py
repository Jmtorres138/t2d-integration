...
home_dir = "/well/got2d/jason/projects/t2d-integration/fgwas/diagram_hrc/peripheral-tissues/single/adipose/"
in_dir=home_dir+"fgwas_input/"
out_dir = home_dir + "fgwas_output/"
input_file=in_dir+"diagram_hrc.fgwas.gz" # Optional: Run 01.1 script and use this for abbreviated annotations: in_dir+"diagram_hrc.renamed.fgwas.gz"
...
def step1():
    ...
    for annot in annot_list:
        job_file = job_dir+"adi_"+annot+".sh"
...
%s
echo "end time" `date`
        ''' % (annot, log_dir,"adi_"+annot,log_dir,"adi_"+annot, command)
        fout.write(script)
        fout.close()
        call = ["qsub", job_file]
        ...
        if os.path.exists(out_path) == True and os.stat(out_path).st_size == 0:
            sp.check_call(call)
    job_list = moniter_rescomp_jobs.get_job_ids("adi_")
...
def step2(sig_list):
    ...
    for annot in sorted_annot[1:]:
        annot = annot[0]
        #print annot
        job_file = job_dir+"adi_"+top_annot+"-"+annot+".sh"
        fout=open(job_file,'w')
        if annot == "distance_tss":
...
%s
echo "end time" `date`
        ''' % (top_annot+"-"+annot, log_dir,"adi_"+top_annot+"-"+annot,
        log_dir,"adi_"+top_annot+"-"+annot, command)
        fout.write(script)
        fout.close()
        call = ["qsub", job_file]
...
def run_models(fixed_list,eval_list):
    ...
    for annot in eval_list:
        job_file = job_dir+"adi_"+fixed_name+"-"+annot+".sh"
        fout=open(job_file,'w')
        if annot == "distance_tss":
            command_list = [fgwas, "-i", input_file, "-cc",
                            "-dists", annot+":"+home_dir+"dist_model",
...
%s
echo "end time" `date`
        ''' % (fixed+"-"+annot, log_dir,"adi_"+fixed+"-"+annot,
        log_dir,"adi_"+fixed+"-"+annot, command)
        fout.write(script)
        fout.close()
        call = ["qsub", job_file]
...
def step4(model_list):
    ...
    for p in p_list:
        job_file = job_dir+"adi_"+model_name+"-"+p+".sh"
        fout=open(job_file,'w')
...
%s
echo "end time" `date`
        ''' % (model_name+"-p"+p, log_dir,"adi_"+model_name+"-p"+p,
        log_dir,"adi_"+model_name+"-p"+p, command)
        fout.write(script)
        fout.close()
        call = ["qsub", job_file]
