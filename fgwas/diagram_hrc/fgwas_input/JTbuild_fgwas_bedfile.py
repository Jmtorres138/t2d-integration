...
cur_dir = "/well/got2d/jason/projects/t2d-integration/fgwas/diagram_hrc/fgwas_input/"
fgwas_head_list = ["CHR","POS0","POS","SNPID","F","Z","PVAL","NCASE","NCONTROL"]
annot_bed_file = "/well/got2d/jason/reference/islet_atac_chromstates_stretch_genomic.bed"
gwas_bed_file = "/well/got2d/jason/reference/gwas/diagram_hrc/diagram_hrc.bed"
outfile = cur_dir+"diagram_hrc.fgwas"
...
def bed_intersect(gwas_bed,annot_bed):
    sys.stdout.write("\nBedtools intersect...\n")
    command_list1 = ["cat",gwas_bed,"|","cut","-f","1,2,3,4",">",cur_dir+"temp.bed"]
    sp.check_call(" ".join(command_list1),shell=True)
    command_list2 = [bedtools, "intersect", "-a", cur_dir+"temp.bed", "-b", annot_bed,
                    "-wb", ">", cur_dir+"intersect.temp"]
    sp.check_call(" ".join(command_list2),shell=True)

def build_dic(dic):
    fin = open(cur_dir+"intersect.temp",'r')
    for line in fin:
        l = line.strip().split()
...
def build_matrix_file(gwas_bed, annot_list, dic, fgwas_head_list):
    ...
    sys.stdout.write("\nWriting Matrix file\n")
    fin = open(gwas_bed,'r')
    fout = open(cur_dir+"intermediate.fgwas",'w')
    ...
    fin.close()
    fout.close()
    command_list = ["gzip", cur_dir+"intermediate.fgwas"]
    sys.stdout.write("\nCompressing file...\n")
...
def main():
    ...
    process_file(cur_dir+"intermediate.fgwas.gz",outfile)
