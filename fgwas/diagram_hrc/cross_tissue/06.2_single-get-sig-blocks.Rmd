...
get_loci_blocks <- function(tissue){
  ...
  save.prefix <- fgwas.output.dir %&% "fgwas_blk-t2d_loci_151"

  loc.df <- fread(eur.dir%&%"t2d-loci-regions.txt")
  ...
  saveRDS(object=sig.blk.df,file= save.prefix%&%".df.RDS")
...
for (tiss in tiss.vec){
  print(tiss)
  loc.blk.df <- get_loci_blocks(tiss)
}
...
# Subset fgwas SNP output file to SNPs in significant blocks
