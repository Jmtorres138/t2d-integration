...
in_dir=home_dir+"intermediate_files/"
out_dir = home_dir + "fgwas_output/"
input_file=in_dir+"fgwas-metabo_input27.txt.gz"
job_dir=home_dir+"jobs/"
log_dir=home_dir+"logs/"
if os.path.isdir(out_dir)==False:
    os.mkdir(out_dir)

def step1(start_index=13):
    ...
    for annot in annot_list:
        ...
        if annot == "distance_tss":
            command_list = [fgwas, "-i", input_file, "-cc", "-fine", "-dists",
                            annot+":"+home_dir+"dist_model", "-o", out_dir+annot]
        else:
            command_list = [fgwas, "-i", input_file, "-cc", "-fine", "-w",
...
def step2(start_index=13,first_go=True):
...
def step3(top_annot, top_val,start_index=13):
    fin1 = gzip.open(input_file,'rb')
    annot_list = fin1.readline().strip().split()[start_index:]
    fin1.close()
...
def main():
    ...
    ### Step 3
    #iter1 = step3(top_annot, top_val)
    #iter2 = step3(iter1[0], iter1[1])
    #iter3 = step3(iter2[0], iter2[1])
    #iter4 = step3(iter3[0], iter3[1])
    ## Note: keep running iterations until additional annotations no longer
    ## improve the model (using ln(lk) or AIC criterion above)

    ### Step 4
    ## There are four annotations in the resulting model (shown below)
    ##model_list = ["islet_atac", "transcript", "islet_state8", "islet_state1"]
    #model_list = ["intron", "islet_state9", "cds", "utr_3"]
