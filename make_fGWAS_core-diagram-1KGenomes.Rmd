...
build_chunk <- function(refnum){
  ...
  nam <- gsub("Credset_99_1000G_","",fname)
  ...
  LEADSNP <- rep(nam,dim(df)[1])
  df <- cbind(df,Z,SE,P,LEADSNP)
...
build_fgwas_core <- function(){
  ...
  out.df$LEADSNP <- as.character(out.df$LEADSNP)
  out.df$LEADSNP <- gsub("chr:","chr",out.df$LEADSNP)
  saveRDS(out.df,file=save.dir%&%"diagram_1KGenomes_fGWAS-core.v2.df.RDS")
  return(out.df)
}
